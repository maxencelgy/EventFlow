...
@section('title', 'Modifier - ' . $participant->full_name)
...
<div class="animate-in max-w-2xl">
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <a href="{{ route('participants.show', $participant) }}" class="hover:text-gray-900">{{ $participant->full_name }}</a>
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
        <span class="text-gray-900">Modifier</span>
    </nav>
    
    <h1 class="text-3xl font-bold text-gray-900 mb-8">Modifier le participant</h1>
    ...
    <form action="{{ route('participants.update', $participant) }}" method="POST">
            ...
            <div class="p-6 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" id="organization" name="organization" value="{{ old('organization', $participant->organization) }}"
                            class="w-full px-4 py-3.5 text-gray-900 bg-gray-50 border border-gray-200 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400">
                    </div>
                    <div>
                        <label for="function" class="block text-sm font-medium text-gray-700 mb-2">Fonction</label>
                        <input type="text" id="function" name="function" value="{{ old('function', $participant->function) }}"
                            class="w-full px-4 py-3.5 text-gray-900 bg-gray-50 border border-gray-200 rounded-xl transition-all focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400">
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6 flex items-center gap-4">
            ...
            <a href="{{ route('participants.show', $participant) }}" class="px-6 py-4 text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors">
                Annuler
            </a>
        </div>
    </form>
