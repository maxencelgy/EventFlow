...
<div class="animate-in">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="mt-1 text-gray-500">{{ $event->date->format('d F Y') }} • {{ $event->start_time }} - {{ $event->end_time }}</p>
        </div>
        <a href="{{ route('events.show', $event) }}" class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-full hover:bg-gray-200 transition-all">
            ← Retour
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="relative overflow-hidden p-6 rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-400 text-white">
            <div class="relative z-10">
                <div class="text-4xl font-bold">{{ $statistics['present'] }}</div>
        ...
        <div class="relative overflow-hidden p-6 rounded-2xl bg-gradient-to-br from-red-500 to-red-400 text-white">
            <div class="relative z-10">
                <div class="text-4xl font-bold">{{ $statistics['absent'] }}</div>
        ...
        <div class="relative overflow-hidden p-6 rounded-2xl bg-gradient-to-br from-gray-800 to-gray-600 text-white">
            <div class="relative z-10">
                <div class="text-4xl font-bold">{{ $statistics['total'] > 0 ? round(($statistics['present'] / $statistics['total']) * 100) : 0 }}%</div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-6">
        <form action="{{ route('checkin.search', $event) }}" method="GET" class="flex gap-4">
            ...
            @if(isset($query) && $query)
                <a href="{{ route('checkin.index', $event) }}" class="px-6 py-3.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-all">
                    Effacer
                </a>
            @endif
        </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="font-semibold text-gray-900">Participants ({{ $registrations->count() }})</h2>
        </div>
        
        @if($registrations->count() > 0)
        <div class="divide-y divide-gray-50">
            @foreach($registrations->sortBy(fn($r) => $r->status === 'present' ? 1 : 0) as $registration)
            <div class="flex items-center justify-between px-6 py-4 {{ $registration->status === 'present' ? 'bg-emerald-50/50' : '' }} hover:bg-gray-50 transition-colors">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold 
                        {{ $registration->status === 'present' ? 'bg-emerald-100 text-emerald-700' : 'bg-gradient-to-br from-gray-600 to-gray-800 text-white' }}">
                        {{ strtoupper(substr($registration->participant->first_name, 0, 1) . substr($registration->participant->last_name, 0, 1)) }}
                    </div>
                    <div>
                        <p class="font-semibold text-gray-900">{{ $registration->participant->full_name }}</p>
                ...
                <div class="flex items-center gap-4">
                    @if($registration->checked_in_at)
                        <span class="text-sm text-gray-500">{{ $registration->checked_in_at->format('H:i') }}</span>
                    @endif
                    
                    @if($registration->status === 'inscrit')
                        <div class="flex gap-2">
                            <form action="{{ route('checkin.checkin', [$event, $registration]) }}" method="POST">
                                @csrf
                            ...
                            <form action="{{ route('checkin.absent', [$event, $registration]) }}" method="POST">
                                @csrf
                    ...
                    @elseif($registration->status === 'present')
